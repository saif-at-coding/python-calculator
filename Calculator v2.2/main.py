...
while True:
    try:
        choice = (
            input(
                "\nEnter 'M', '1' or 'H' for Help/Menu.\nChoose an operation (1-26 or Symbols)\n>> "
            )
            .replace(" ", "")
            .upper()
        )
        ...
        elif choice in ["24", "25", "26", "LOGN", "LOG10", "LOG2"]:
            value = float(input("Enter the value: "))
            try:
                if choice in ["24", "LOGN"]:
                    result = f"log({value}) = {operations.logn(value)}"
                elif choice in ["25", "LOG10"]:
                    result = f"log_10_({value}) = {operations.log10(value)}"
                else:
                    result = f"log_2_({value}) = {operations.log2(value)}"
            except ValueError as e:
                print(e)
                result = f""
            utils.add_to_history(result)
        ...
        elif choice in ["23", "LOG"]:
            value = float(input("Enter the value: "))
            base = float(input("Enter the log base: "))
            try:
                result = f"log_{base}_({value}) = {operations.log(value, base)}"
            except ValueError as e:
                print(e)
                result = f""
            utils.add_to_history(result)
        ...
        print(f"Expected a number, User!")
    except OverflowError:
        print(f"That's way too big, User!")
